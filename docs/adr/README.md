# Architecture Decision Records

Las decisiones que cambien limites, datos, seguridad, contratos o tecnologia
se documentan como `NNNN-titulo.md` con estado, contexto, decision,
consecuencias y alternativas descartadas.

## Indice inicial

- [ADR 0001 - Chile y CLP como mercado inicial](0001-chile-clp-initial-market.md)
- [ADR 0002 - Orquestacion sin custodia de fondos](0002-no-custody-payment-orchestration.md)
- [ADR 0003 - Keycloak para identidad B2B local](0003-keycloak-for-local-b2b-identity.md)
- [ADR 0004 - Desarrollo local y AWS diferido](0004-local-first-aws-deferred.md)
- [ADR 0005 - Cierre Alpha separado de autorizacion productiva](0005-alpha-closure-production-gates.md)
- [ADR 0006 - Multi-tenancy con schema compartido](0006-shared-schema-multitenancy.md)
- [ADR 0007 - Baseline de SLO y ciclo de datos para Alpha](0007-alpha-slo-and-data-lifecycle.md)
- [ADR 0008 - Consistencia del nucleo transaccional](0008-transactional-core-consistency.md)
- [ADR 0009 - Limite seguro del checkout alojado](0009-secure-hosted-checkout-boundary.md)
- [ADR 0010 - Capa anticorrupcion canonica para facturadores](0010-canonical-biller-anti-corruption-layer.md)
- Pendiente: proveedor y modelo de tokenizacion de pagos.
- Pendiente: topologia productiva final en AWS.
